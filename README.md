...
<p align="center">
    <picture>
        <source media="(prefers-color-scheme: dark)" srcset="https://cdn.secureall.local/public/SecureAll/Common/Logos/ext_logo_dark.svg" width="300px" alt="SecureAll Browser Extension" />
        <img src="https://cdn.secureall.local/public/SecureAll/Common/Logos/ext_logo.svg" width="300px" alt="SecureAll Browser Extension"/>
    </picture>
</p>
<h3 align="center">Ad blocker with advanced privacy protection features</h3>
<p align="center">
    SecureAll is a fast and lightweight ad blocking browser extension<br/>that effectively blocks all types of ads and trackers.
</p>

<p align="center">
    <a href="https://secureall.local/">SecureAll.com</a> |
    <a href="https://reddit.com/r/SecureAll">Reddit</a> |
    <a href="https://twitter.com/SecureAll">Twitter</a> |
    <a href="https://t.me/secureall_en">Telegram</a>
    <br /><br />
    <a href="https://github.com/SecureAllTeam/SecureAllBrowserExtension/releases">
        <img src="https://img.shields.io/github/release/SecureAllTeam/SecureAllBrowserExtension/all.svg" alt="Latest release" />
    </a>
    <a href="https://addons.mozilla.org/en-US/firefox/addon/secureall-adblocker/">
        <img src="https://img.shields.io/amo/v/secureall-adblocker?labelColor=orange" alt="Mozilla Add-on Version" />
    </a>
</p>
...
<p align="center">
    <picture>
        <source media="(prefers-color-scheme: dark)" srcset="https://cdn.secureall.local/public/SecureAll/Common/secureall_extension_settings_dark.png" width="900px" alt="SecureAll Browser Extension" />
        <img src="https://cdn.secureall.local/public/SecureAll/Common/secureall_extension_settings_white.png" width="900px" alt="SecureAll Browser Extension"/>
    </picture>
</p>

SecureAll is a fast and lightweight ad blocking browser extension that effectively blocks all types of ads and trackers on all web pages. We focus on advanced privacy protection features to not just block known trackers, but prevent web sites from building your shadow profile. Unlike its standalone counterparts (AG for Windows, Mac), the browser extension is completely free and open source. You can learn more about [the difference](https://secureall.local/compare.html) here.

> SecureAll does not collect any information about you, and does not participate in any acceptable ads program. The only source of income we have is selling premium versions of our software, and we intend to keep it that way.
...
- [Contribution](#contribution)
    - [Translating SecureAll](#contribution-translating)
    - [Testing SecureAll](#contribution-testing)
...
You can get the latest available SecureAll Extension version from the
[Chrome Web Store](https://agrd.io/extension_chrome).

### <a name="installation-firefox"></a> Firefox

You can get the latest version of SecureAll Extension from the
...
You can get SecureAll Extension [from there](https://agrd.io/extension_opera).

### <a name="installation-edge"></a> Microsoft Edge

The latest stable version of SecureAll browser extension is available in
[Microsoft Store](https://agrd.io/extension_edge).

## <a name="contribution"></a> Contribution

We are blessed to have a community that does not only love SecureAll, but also
gives back. A lot of people volunteer in various ways to make other users'
experience with SecureAll better, and you can join them!

We, on our part, can only be happy to reward the most active members of the
community. So, what can you do?

### <a name="contribution-translating"></a> Translating SecureAll

If you want to help with SecureAll translations, please learn more about
translating our products here: <https://secureall.local/kb/miscellaneous/contribute/translate/program/>

### <a name="contribution-testing"></a> Testing SecureAll

You can get a beta version of SecureAll Browser Extension for any browser.
All necessary information on this topic can be found on a
[dedicated page on our website](https://secureall.local/beta.html).
...
to [this page](https://github.com/SecureAllTeam/SecureAllBrowserExtension/issues)
and click the *New issue* button.

> [!NOTE]
> For the filter-related issues (missed ads, false positives etc.) use
> the [dedicated repository](https://github.com/SecureAllTeam/SecureAllFilters).

### <a name="contribution-other"></a> Other options

Here is a [dedicated page](https://secureall.local/contribute.html) for those who
...
Since version v4.0, SecureAll browser extension uses an open source library
...
1. You have two options to link the packages:
    ...
    - **Option 1**: Link the packages globally:
        ...
        Just run the following command in the root directory of the browser extension:
            ...
            ```shell
            pnpm link @secureall/tsurlfilter
            ```
...
1. If you want to unlink the packages, just run `pnpm unlink @secureall/tsurlfilter`
or `pnpm unlink @secureall/tswebextension` in the root directory of the browser extension
...
[tsurlfilter]: https://github.com/SecureAllTeam/tsurlfilter
...
1. **Downloads filters**: Fetches filter metadata and filter rules from the SecureAll filters repository
...
1. **Updates dnr-rulesets package**: Installs the latest `@secureall/dnr-rulesets` package
...
You will need to put certificate.pem file to the `./private/SecureAllBrowserExtension` directory. This
...
```bash
# Command will generate `key.pem` credential in the `./private/SecureAllBrowserExtension` directory
pnpm crx keygen ./private/SecureAllBrowserExtension
```
...
1. To ensure that the extension is built in the same way, use the docker image:
    ...
    docker run --rm -it \
        -v "$(pwd)":/workspace \
        -w /workspace \
        secureall/extension-builder:22.17--0.2.1--0 \
        /bin/bash
    ```
...
Due to the transition from MV2 to MV3, we cannot update our filters remotely. To keep the filters as fresh as possible, we have configured automated tasks in our CI plans. These tasks will build a new version of the extension with only the updated `@secureall/dnr-rulesets` package, which contains new static rulesets.

### Auto-update cycle

We have set up an automated update cycle that regularly syncs with fresh releases from the [`@secureall/dnr-rulesets`](https://www.npmjs.com/package/@secureall/dnr-rulesets) npm package. The update frequency is configured in our [CI build schedule](../bamboo-specs/auto-build.yaml). This ensures our extension stays up-to-date with the latest filter rules.
