...
import { SECUREALL_SETTINGS_KEY } from '../../../../Extension/src/common/constants';
...
describe('Settings Api', () => {
    ...
    describe('Reverting settings to default values when: ', () => {
        it('one of fields is omitted', async () => {
            ...
            delete deepCopy[SettingOption.DisableShowSecureAllPromoInfo];
            storage = mockLocalStorage({
                [SECUREALL_SETTINGS_KEY]: deepCopy,
            });
            await SettingsApi.init();

            const settings = await storage.get(SECUREALL_SETTINGS_KEY);
            expect(settings).toStrictEqual({ [SECUREALL_SETTINGS_KEY]: defaultSettings });
        });

        it('entire settings object is omitted', async () => {
            storage = mockLocalStorage({
                [SECUREALL_SETTINGS_KEY]: {},
            });
            await SettingsApi.init();
    ...
    describe('reads and writes setting storage data', () => {
        beforeEach(async () => {
            storage = mockLocalStorage({ [SECUREALL_SETTINGS_KEY]: defaultSettings });
            await SettingsApi.init();
        });

        it('Inits settings cache', () => {
            expect(browser.storage.local.get.calledOnceWith(SECUREALL_SETTINGS_KEY)).toBe(true);
